...
MODULE_AUTHOR("Holovenets Ruslan IV-83");
MODULE_DESCRIPTION("Hello world (LAB6)");
...
MODULE_PARM_DESC(amount, "Amount (Hello world)");
...
while (head != NULL && amount != 0) {
	var = head;
	pr_info("Time in ns: %lld", var->time);
	head = var->next;
	kfree(var);
}
pr_info("");
